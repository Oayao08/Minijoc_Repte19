...
// ─── UI: IDs corregidos (sin "#") y adaptados a los elementos reales de joc.html ───
const ui = {
  ...
  popupConfirm: document.getElementById('popup-button'),   // botón "ENTENDIDO"
...
// ─── ASSETS: rutas ajustadas a la estructura real del proyecto ───
const ASSETS = {
...
const POPUPS = {
  ...
  mobile: {
    ...
    body:  'Usar el mòbil mentre circules amb patinet és molt perillós i il·legal. Perd concentració i temps de reacció.',
    button: 'ENTÈS'
  },
  headphones: {
...
function loadImage(key, src) {
  ...
  img.onerror = () => { console.warn('No encontrado:', src); images[key] = null; };
}
...
function createState() {
  return {
    running:   true,
    paused:    true,   // empieza pausado hasta que el jugador cierra el popup inicial
    gameEnded: false,
    elapsed:   0,
    score:     0,
    goal:      100,
...
function showPopup(kind) {
  ...
  if (ui.popupIcon)  { ui.popupIcon.src = data.icon || ''; ui.popupIcon.style.display = data.icon ? '' : 'none'; }
...
function spawnItem(type) {
  const lastX = state.objects.length
    ? Math.max(...state.objects.map(o => o.x))
    : -9999;
...
function updateDifficulty(dt) {
  ...
  const delta = state.keys.right && state.player.vx > 10
...
function drawParallax() {
  ...
  for (let i = -1; i < 4; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(x, 380); ctx.lineTo(x + 170, 170); ctx.lineTo(x + 330, 380); ctx.closePath(); ctx.fill();
  }
  ...
  for (let i = -1; i < 6; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(x, 390); ctx.lineTo(x + 120, 250); ctx.lineTo(x + 220, 390); ctx.closePath(); ctx.fill();
  }
  ...
  for (let i = -1; i < 20; i++) {
    ...
    ctx.beginPath(); ctx.moveTo(x, ROAD_Y + 45); ctx.lineTo(x + 40, ROAD_Y + 45); ctx.stroke();
  }
}

function drawPlayer() {
  const p     = state.player;
  const swing = Math.sin(p.animT * 18) * 3;
...
window.addEventListener('keydown', e => {
  ...
  if (e.key.toLowerCase() === 'p' && !state.gameEnded && !state.activePopup) {
    ...
    if (state.paused) {
      // Pausa manual: muestra popup reutilizando overlay
      if (ui.overlay)      ui.overlay.classList.remove('hidden');
...
if (ui.restartBtn)   ui.restartBtn.addEventListener('click', () => window.location.reload());

// ─── ARRANQUE ───
updateHud();
showPopup('initial');        // Popup de bienvenida (pausa hasta cerrarlo)
requestAnimationFrame(loop); // Un único requestAnimationFrame
